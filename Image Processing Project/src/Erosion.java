...
/* This program performs an erosion operation on a black and white image;
 * that is, every black pixel adjacent (diagonals included) to a white
 * background pixel is turned white. A one-row-by-one-column white background
 * is maintained for the purposes of this program.
 */
...
public class Erosion
{
   ...
   // constructor
   public Erosion(String userFileString) throws Exception
   {
      ...
      // performs erosion
      imagePixels = erodeImage(imagePixels, imageHeight, imageWidth);
      
      // writes result into new image file
      writeImageToFile(fillNewImage());
   }
   
   // erosion method - deletes all black pixels adjacent to white pixels
   public static int[][] erodeImage(int[][] pixelArray, int height, int width)
   {
      // identifies pixels to be eroded
      for (int r = 1; r < height - 1; r++) // background is always present
      {
         for (int c = 1; c < width - 1; c++)
         {
            if (pixelArray[r][c] == BLACK_PIXEL &&
                  (pixelArray[r - 1][c - 1] == WHITE_PIXEL ||
                  pixelArray[r - 1][c] == WHITE_PIXEL ||
                  pixelArray[r - 1][c + 1] == WHITE_PIXEL ||
                  pixelArray[r][c + 1] == WHITE_PIXEL ||
                  pixelArray[r + 1][c + 1] == WHITE_PIXEL ||
                  pixelArray[r + 1][c] == WHITE_PIXEL ||
                  pixelArray[r + 1][c - 1] == WHITE_PIXEL ||
                  pixelArray[r][c - 1] == WHITE_PIXEL))
            {
               // sets each pixel to be eroded to an intermediate value
               pixelArray[r][c] = 1;
            }
         }
      }
      
      // sets all pixels to be eroded to white
      for (int row = 1; row < height - 1; row++)
      {
         for (int col = 1; col < width - 1; col++)
         {
            ...
            {
               pixelArray[row][col] = WHITE_PIXEL;
            }
         }
      }
   ...
   {
      outputFileName = "outputErosion.png";
      try
      {
   ...
   {
      Erosion newErosion = new Erosion("ExampleImage1.png");
      // retrieves output file name and displays image in Java GUI
      String newImageFile = newErosion.getOutputFileName();
      JFrame newFrame = new JFrame("Eroded Image");
